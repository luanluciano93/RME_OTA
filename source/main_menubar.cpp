...
namespace SearchDuplicatedItems {
	struct condition {
		...
		void operator()(Map& map, Tile* tile, Item* item, int64_t done) {
			if (done % 0x8000 == 0) {
				g_gui.SetLoadDone((uint32_t)(100 * done / map.getTileCount()));
			}
...
namespace RemoveDuplicatesItems {
	struct condition {
		bool operator()(Map& map, Tile* tile, Item* item, int64_t removed, int64_t done) {
			if (done % 0x8000 == 0) {
				g_gui.SetLoadDone((uint32_t)(100 * done / map.getTileCount()));
			}
...
void MainMenuBar::RemoveDuplicatesItems(bool onSelection/* = false*/) {
	...
	if(ok == wxID_YES) {
		...
		int64_t removed = RemoveItemDuplicateOnMap(g_gui.GetCurrentMap(), func, onSelection);
